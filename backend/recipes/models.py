...
class Recipe(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
                               related_name='recipes', help_text='Автор '
                                                                 'рецепта')
    ...
    cooking_time = models.IntegerField(help_text='Время приготовления ('
                                                 'минуты)')
    tags = models.ManyToManyField('Tag', related_name='recipes', blank=True,
                                  help_text='Теги рецепта')
    ingredients = models.ManyToManyField(
        'Ingredient', through='IngredientAmountPerRecipe',
        related_name='recipes',
        help_text='Ингредиенты рецепта')

    short_link = models.CharField(unique=True, max_length=100, )
...
class Ingredient(models.Model):
    name = models.CharField(max_length=100, unique=True, help_text='Название')
    measurement_unit = models.CharField(max_length=100, help_text='Единица '
                                                            'измерения')
...
class Cart(models.Model):
    ...
    recipes = models.ManyToManyField('Recipe', related_name='carts',
